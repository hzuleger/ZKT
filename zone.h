...
typedef	struct	Zone {
	...
	const	zconf_t	*conf;	/* ptr to config */	/* TODO: Should this only a ptr to a local config ? */
...
extern	int	zone_readdir (const char *dir, const char *zone, const char *zfile, zone_t **listp, const zconf_t *conf);
